...
const invalidStatus = await admin.json(`/api/registrations/${attendeeRegistration.id}`, "PATCH", { status: "INVALID" });
assert.equal(invalidStatus.response.status, 422);
assert.ok(invalidStatus.body.fields.status);
...
const completed = await admin.json(`/api/events/${created.body.event.id}`, "PATCH", { status: "COMPLETED", startDateTime: "2020-01-01T10:00:00.000Z", endDateTime: "2020-01-01T11:00:00.000Z" });
assert.equal(completed.response.status, 200);
const completedRegistrations = await attendee.request("/api/registrations");
const completedRegistration = completedRegistrations.body.registrations.find((item) => item.event.id === created.body.event.id);
...
const feedback = await attendee.json("/api/feedback", "POST", { eventId: created.body.event.id, rating: 5, comment: "The completed test event was useful." });
assert.equal(feedback.response.status, 201);
const duplicateFeedback = await attendee.json("/api/feedback", "POST", { eventId: created.body.event.id, rating: 4, comment: "This duplicate should be rejected." });
assert.equal(duplicateFeedback.response.status, 409);
assert.equal(duplicateFeedback.body.code, "FEEDBACK_ALREADY_SUBMITTED");
const deleted = await admin.json(`/api/events/${created.body.event.id}`, "DELETE", {});
assert.equal(deleted.response.status, 409);

console.log("End to end API checks passed, including profile, registration status, and feedback workflows.");
