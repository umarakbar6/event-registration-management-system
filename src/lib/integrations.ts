...
export async function emitRegistrationWebhook(payload: RegistrationWebhookPayload) {
  ...
  try {
    await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", ...(process.env.N8N_WEBHOOK_SECRET ? { "x-gatherly-secret": process.env.N8N_WEBHOOK_SECRET } : {}) }, body: JSON.stringify({ type: "registration.created", source: "gatherly", payload }), signal: controller.signal, cache: "no-store" });
  } catch (error) {
